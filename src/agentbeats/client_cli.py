...
async def main():
    ...
    # Calculate timeout based on max_steps if available
    max_steps = req.config.get("max_steps") if isinstance(req.config, dict) else None
    timeout = None
    if max_steps is not None and isinstance(max_steps, int):
        # For green agent, we need time for both blue agent execution AND evaluation
        # So we use a multiplier: max_steps * 30 seconds per step (more generous)
        timeout = calculate_timeout_from_max_steps(max_steps, seconds_per_step=30, buffer=180)
        print(f"⏱️  Using timeout: {timeout}s (based on max_steps={max_steps})")

    msg = req.model_dump_json()
    await send_message(msg, green_url, streaming=True, consumer=event_consumer, timeout=timeout)
